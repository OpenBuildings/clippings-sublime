...
class TestRunCommand(sublime_plugin.TextCommand):
	...
	def run(self, edit):
		...
		self.view.window().run_command('exec', {'cmd': ['vagrant ssh -c "cd {0}; phpunit {1}"'.format(d, f)], 'shell': True, 'syntax': 'Packages/clippings-sublime/SimplePHPUnit.hidden-tmLanguage'} )
